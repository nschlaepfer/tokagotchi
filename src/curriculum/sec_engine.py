...
class SECEngine:
    ...
    def add_task(self, spec: TaskSpec, *, status: str = "active") -> None:
        ...
        self._bank[spec.task_id] = {
            ...
            "avg_steps": 0.0,
            "last_attempted": None,
            "status": status,  # active | retired | shelved | draft_untrusted
        }
    ...
    def add_external_tasks(
        ...
        *,
        ...
        require_static_validation: bool = True,
    ) -> int:
            ...
            try:
                ...
                metadata = {
                    **(td.get("metadata", {}) or {}),
                    ...
                    "oracle_trusted": allow_teacher_generated_tests,
                }
                spec = TaskSpec(
                    task_id=td.get("task_id", f"{source}_{_time.strftime('%H%M%S')}_{added}"),
                    task_type=_parse_task_type(td.get("task_type", "open_ended_optimization")),
                ...
                has_teacher_oracle = bool(spec.test_commands or spec.expected_output)
                if has_teacher_oracle and not allow_teacher_generated_tests:
                    logger.warning(
                        "Blocked external task %s from %s: teacher-generated oracle is disabled",
                        spec.task_id,
                        source,
                    )
                    continue

                if require_static_validation:
                    validation = validator.validate_static(spec)
                    spec.metadata["static_validation"] = validation.to_dict()
                    if not validation.valid:
                        logger.warning(
                            "Blocked external task %s from %s: static validation failed: %s",
                            spec.task_id,
                            source,
...
def _parse_task_type(value: Any) -> TaskType:
    """Parse serialized or teacher-provided task type values safely."""
